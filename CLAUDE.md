...
- **Assignment is per-deliverable, not per-team** (`resolveAssignments` in `lib/roster.js`): a meeting can name a task owner who was never even on the call ("let Shweta take the video") — that person gets auto-created by name (with a best-effort Slack display-name match) and assigned that specific task, without swallowing every other task on the same team. Only a deliverable with **no** named owner falls back to the old team-wide logic (present attendee on that team, else lowest ticket count, else — design only — ask the team lead to pick between 2+ present designers).
