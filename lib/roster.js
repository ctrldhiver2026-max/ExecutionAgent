 ...
 * Meet's DOM only exposes display names, not emails, and display names can
 * drift from what's in the roster ("Hari K" vs "Hari krishnan") — so when a
 * direct name match fails, fall back to the calendar event's attendee list
 * (real emails from the invite) as a second path: match the captured name
 * against a calendar attendee's name to get their email, then look that
 * email up in the roster. Unmatched attendees are kept (name only) so they
 * still display on the dashboard/Slack DM, just without a slack_id
 * (harmless — channel invite filters those out).
 *
 * @param calendarAttendees  optional [{name, email}] from the correlated
 *   calendar event (lib/google.js findEventByMeetCode) — omit if unavailable.
 */
export async function resolveAttendees(attendeeNames = [], calendarAttendees = []) {
  ...
  return attendeeNames.map((name) => {
    let match = matchRosterByName(name, roster);
    if (!match) {
      const calendarEmail = emailByCalendarName.get(normalize(name));
      if (calendarEmail) {
        const target = normalize(calendarEmail);
        match = roster.find((r) => normalize(r.email) === target) || null;
      }
    }
    return match
