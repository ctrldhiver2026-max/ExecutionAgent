...
export function matchRosterByName(name, roster) {
  ...
  // A meeting names someone by first name only ("Shweta will handle the
  // video") while their roster row carries their full name ("Shweta B",
  // captured from an earlier calendar-invite resolution) — match on the
  // first word, but only when it's unique, so two different "Priya"s on
  // the roster don't collide (same safety rule as the exact-name check).
  const firstNameMatches = roster.filter((r) => normalize(r.name).split(" ")[0] === n);
  ...
  // instead of reusing her). Only trusted when exactly ONE roster row is
  // close (distance <= 2, on names of comparable length) — same
  // refuse-to-guess rule as the tier above, so this can't misfire on two
  // genuinely different short names.
  if (n.length >= 4) {
    const close = roster.filter((r) => {
      const rn = normalize(r.name).split(" ")[0];
      if (!rn || rn.length < 4 || Math.abs(rn.length - n.length) > 2) return false;
      return levenshtein(rn, n) <= 2;
    });
    if (close.length === 1) {
