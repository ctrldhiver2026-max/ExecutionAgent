...
// to repeat per lookup; warm serverless containers reuse this. TTL'd so
// people who join the workspace mid-event become visible without waiting
// for a container cold start.
let cachedUserList = null;
let cachedUserListAt = 0;
const USER_LIST_TTL_MS = 10 * 60 * 1000;

async function listAllUsers() {
  if (cachedUserList && Date.now() - cachedUserListAt < USER_LIST_TTL_MS) return cachedUserList;
  const users = [];
  let cursor;
  do {
 ...
 * and UNIQUE match only — verified live (2026-07-10) that this company
 * workspace has two different people whose display name is exactly
 * "Hari"; first-match-wins would DM a stranger. Ambiguous names return
 * null and fall through to the manual People-tab path instead.
 */
export async function lookupUserByName(name) {
  ...
  const nameFields = (u) => [
    (u.profile?.real_name || u.real_name || "").trim().toLowerCase(),
    (u.profile?.display_name || "").trim().toLowerCase(),
  ];

  const exact = users.filter((u) => nameFields(u).some((n) => n && n === target));
  if (exact.length === 1) {
    const match = exact[0];
    return { id: match.id, name: match.profile?.real_name || match.real_name || name };
  }
  if (exact.length > 1) {
    console.log(`[slack] name "${name}" is ambiguous (${exact.length} workspace members match exactly) — refusing to guess`);
    return null;
  }

  // No exact match — someone spoken of by first name only ("Shweta will
  // handle the video") vs their full Slack real_name ("Shweta B"). Match on
  // first word, unique-match-only (same safety rule as the exact check
  // above): two "Priya"s in the workspace must never resolve to either one.
  const firstNameMatches = users.filter((u) => nameFields(u).some((n) => n && n.split(" ")[0] === target));
  if (firstNameMatches.length === 1) {
    const match = firstNameMatches[0];
    return { id: match.id, name: match.profile?.real_name || match.real_name || name };
  }
  if (firstNameMatches.length > 1) {
    console.log(`[slack] name "${name}" matches multiple workspace members by first name — refusing to guess`);
  }
  return null;
}
...
/**
 * Review-approval acknowledgment with buttons (2026-07-10) — sent when an
 * assignee tags a reviewer for a specific subtask. "Approve" and "Add
 * comments" both carry the pending_reviews row id in their value, same
 * pattern as sendConfirmationDm's confirmationId. Plain-text "approved"
 * still works as a fallback (api/slack/events.js's isApprovalMessage), this
 * is a more discoverable primary path, not a replacement.
 */
export async function sendReviewButtons(channel, approverSlackId, subtaskName, pendingReviewId) {
  return postMessage(channel, `Waiting on <@${approverSlackId}> to review "${subtaskName}"`, [
    {
      type: "section",
      text: { type: "mrkdwn", text: `:eyes: <@${approverSlackId}>, *${subtaskName}* is ready for your review.` },
    },
    {
      type: "actions",
...
/**
 * Opens the "Add comments" modal (must be called within Slack's ~3s
 * trigger_id validity window, so keep any work before this call minimal).
 * private_metadata round-trips through Slack unmodified and comes back on
 * view_submission, carrying enough context to route the submitted comment
 * without a second DB lookup keyed off anything guessable.
 */
export async function openReviewCommentModal(triggerId, { pendingReviewId, channelId, subtaskName }) {
  return slackApi("views.open", {
    trigger_id: triggerId,
    view: {
      type: "modal",
      callback_id: "review_comment_modal",
      private_metadata: JSON.stringify({ pendingReviewId, channelId }),
      title: { type: "plain_text", text: "Add feedback" },
      ...
      blocks: [
        {
          ...
          label: { type: "plain_text", text: `Feedback on "${subtaskName.slice(0, 140)}"` },
 ...
 */
export async function postProjectKickoff(channelId, projectName, owners, clickupUrl, attendees = [], summary = "") {
  ...
  // Meeting summary (the extraction's MOM) — a separate block so it reads
  // as its own card rather than getting buried under the assignee list.
  // Slack caps a section's text at 3000 chars; a 2-3 sentence summary is
  // nowhere close, but truncate defensively rather than let a future
  // extraction change (e.g. a longer meeting) silently 400 the whole post.
  if (typeof summary === "string" && summary.trim()) {
    const text = summary.trim();
    const capped = text.length > 2800 ? `${text.slice(0, 2800)}…` : text;
    blocks.push({
      type: "section",
      text: { type: "mrkdwn", text: `:memo: *Meeting summary*\n${capped}` },
    });
  }
  ...
  return postMessage(channelId, `Project ${projectName} created`, blocks);
}
