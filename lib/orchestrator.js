...
async function resolveAssignments(payload, opts) {
  ...
  const designers = (payload.attendees || []).filter((a) => a.team === "design");
  if (designers.length >= 2) {
    // Flow (a): 2+ designers in meeting → ask design manager to pick.
    // Real manager lookup is Charan's roster job — DMing STUB_DESIGN_OWNER
    // as a stand-in "manager" so this path is exercisable before that lands.
    return {
      needsManagerPick: true,
      managerSlackId: process.env.STUB_DESIGN_OWNER,
      designerCandidates: designers.map((d) => ({ name: d.name, slack_id: d.slack_id })),
    };
  }
