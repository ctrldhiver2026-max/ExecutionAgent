...
/**
 * Turn createClickUpTickets()'s flat per-deliverable subtask list into a
 * per-team completion-tracking structure (which teams are involved, in
 * TEAM_ORDER for consistent display, and each team's subtasks + done flags)
 * for the dashboard Timeline and the Slack review-approval flow. Every team
 * starts "active" (2026-07-10: all teams are assigned and can work in
 * parallel from day one — no team waits on another to "unlock" — see
 * lib/clickup.js's TEAM_ORDER comment for why the earlier sequential
 * version was reverted). "status" is kept for backward compatibility with
 * that flow (a team still flips to "done" once all its subtasks are
 * approved) rather than removed outright. Returns null if there's nothing
 * to track (no deliverables, or none with a recognized team).
 */
...
export async function runProjectCreation(confirmation, opts = {}) {
  ...
  // plus the meeting's own summary (the "MOM") so the channel has context
  // and not just a task list ─────────────────────────────────────────────
  await postProjectKickoff(channelId, p.project_name, assignment.owners, clickup.url, p.attendees, p.summary);
