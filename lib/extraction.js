        ...
        items: {
          ...
          properties: {
            ...
            owner_name: {
              type: ["string", "null"],
              description:
                "The SINGLE person named as responsible for this task, exactly as spoken/referred to in " +
                "the transcript — this can be someone NOT in the attendee list (e.g. delegated to someone " +
                "who wasn't on the call). Must always be one name, never a list or a phrase joining " +
                "multiple candidates (e.g. never 'Priya or Raj', never 'Priya and Raj', never 'whoever is " +
                "free between Priya and Raj') — if the transcript settles on one specific person by the " +
                "end of the discussion, use that person even if others were floated earlier; if it never " +
                "actually settles and stays genuinely open between multiple people, use null so it can be " +
                "assigned properly downstream instead of guessing or inventing a combined name. Use null " +
                "also when the transcript never names anyone for this task at all.",
            },
            due_date: {
              type: ["string", "null"],
              description:
  ...
  const res = await fetch(ANTHROPIC_API_URL, {
    ...
    body: JSON.stringify({
      ...
      system:
        ...
        "when the transcript truly never names anyone for that task. Meetings sometimes float multiple " +
        "candidates before settling on one ('let's give it to Aviva' after discussing both Priya and " +
        "Aviva being on leave) — use whoever it actually lands on, never a joined name like 'Priya or " +
        "Aviva'; if it genuinely never lands on one person, use null instead of guessing or combining " +
        "names. If the transcript does not discuss " +
