...
/**
 * Correlate a captured Meet call (its meeting code from the URL, e.g.
 * "heb-wkfg-ctt") back to the calendar event that scheduled it, so the
 * pipeline knows who organized the meeting — that's who gets the "still
 * ON?" confirmation DM. Searches a window around "now" since ingest runs
 * right as the meeting ends (the event's start time is already in the past
 * by then), not in the future like listUpcomingEvents.
 */
export async function findEventByMeetCode(meetCode, windowHoursBack = 6) {
  if (!meetCode) return null;
  const token = await getAccessToken();
  const now = new Date();
  const params = new URLSearchParams({
    timeMin: new Date(now.getTime() - windowHoursBack * 60 * 60_000).toISOString(),
    timeMax: new Date(now.getTime() + 30 * 60_000).toISOString(),
    singleEvents: "true",
    maxResults: "50",
  });
  const res = await fetch(
  ...
  const match = (data.items || []).find(
    (e) => e.conferenceData?.conferenceId === meetCode || e.hangoutLink?.includes(meetCode)
  );
