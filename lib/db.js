...
/**
 * Persist the meeting → ClickUp → Slack channel mapping once the project is
 * created. held_by_id defaults the dashboard's Owner field to whoever
 * confirmed the project (2026-07-10) instead of leaving it unassigned.
 * stage_plan is the sequential department handoff's state (2026-07-10
 * migration, see README) — degrades to creating without it if that column
 * doesn't exist yet, same pattern as listProjects()'s held_by_id fallback.
 */
export async function createProjectRecord({ meeting_id, project_name, clickup_task_id, clickup_url, slack_channel_id, held_by_id, stage_plan }) {
  try {
    const rows = await rest("projects", {
      method: "POST",
      body: { meeting_id, project_name, clickup_task_id, clickup_url, slack_channel_id, held_by_id, stage_plan },
    });
    return rows[0];
  } catch (err) {
    if (String(err.message).includes("stage_plan")) {
      console.error("[db] projects.stage_plan missing (migration not run yet) — creating without it, sequential handoff won't advance");
      const rows = await rest("projects", {
        method: "POST",
        body: { meeting_id, project_name, clickup_task_id, clickup_url, slack_channel_id, held_by_id },
      });
      return rows[0];
    }
    throw err;
...
/**
 * Most recent meetings row for this exact meeting_id, if one was created
 * within the last `windowMinutes` — used by api/meetings/ingest.js to
 * detect a duplicate capture of the SAME real-world call (live incident
 * 2026-07-10: a page reload / duplicate tab-close race posted the same
 * meeting twice, ~5 seconds apart, with different partial attendee lists —
 * two separate meetings rows for one real meeting, visible as a duplicate
 * card on the dashboard, plus a wasted Claude extraction call on the
 * incomplete one). A generous window (default 30 min) safely covers any
 * realistic duplicate-post race without risking treating a genuinely new
 * capture of a reused/recurring Meet code (e.g. the same room days later)
 * as a duplicate.
 */
export async function getRecentMeetingByMeetingId(meeting_id, windowMinutes = 30) {
  if (!meeting_id) return null;
  const rows = await rest("meetings", {
    query: `?meeting_id=eq.${encodeURIComponent(meeting_id)}&select=id,meeting_id,created_at,transcript,attendees,extracted&order=created_at.desc&limit=1`,
  });
  const row = rows[0];
  if (!row) return null;
  const ageMinutes = (Date.now() - new Date(row.created_at).getTime()) / 60000;
  return ageMinutes <= windowMinutes ? row : null;
}

/** Merge a follow-up capture into an existing meeting row instead of creating a visible duplicate — see getRecentMeetingByMeetingId. */
export async function updateMeetingRecord(id, { ended_at, attendees, transcript, extracted }) {
  const body = {};
  if (ended_at !== undefined) body.ended_at = ended_at;
  if (attendees !== undefined) body.attendees = attendees;
  if (transcript !== undefined) body.transcript = transcript;
  if (extracted !== undefined) body.extracted = extracted;
  const rows = await rest("meetings", {
    method: "PATCH",
    body,
    query: `?id=eq.${encodeURIComponent(id)}`,
...
/**
 * Look up which project a Slack channel belongs to — the review-approval
 * flow needs this to know a channel message is even about a tracked
 * project, and (2026-07-10) to read/advance its stage_plan for the
 * sequential department handoff. Degrades to the pre-migration select if
 * stage_plan doesn't exist yet — the caller treats a missing stage_plan the
 * same as "no staging configured" and just skips advancement.
 */
export async function getProjectByChannelId(channel_id) {
  if (!channel_id) return null;
  try {
    const rows = await rest("projects", {
      query: `?slack_channel_id=eq.${encodeURIComponent(channel_id)}&select=meeting_id,project_name,clickup_task_id,slack_channel_id,stage_plan,held_by_id`,
    });
    return rows[0] || null;
  } catch (err) {
    if (String(err.message).includes("stage_plan")) {
      const rows = await rest("projects", {
        query: `?slack_channel_id=eq.${encodeURIComponent(channel_id)}&select=meeting_id,project_name,clickup_task_id,slack_channel_id,held_by_id`,
      });
      return rows[0] || null;
    }
    throw err;
...
/** Persist an updated stage_plan (sequential department handoff advancing) — no-op (logged, non-fatal) if the migration hasn't run, since there's nothing to advance without it. */
export async function updateProjectStagePlan(meeting_id, stage_plan) {
  try {
    const rows = await rest("projects", {
      method: "PATCH",
      body: { stage_plan },
    ...
    if (String(err.message).includes("stage_plan")) {
      console.error("[db] projects.stage_plan missing (migration not run yet) — can't persist stage advancement");
      return null;
    }
    throw err;
  }
...
// The 2026-07-10 migration (README) adds 3 columns to pending_reviews in one
// statement, but PostgREST's error only names whichever column IT happens to
// validate first — for a query that touches more than one of the three
// (e.g. listCompletedReviews filters on completed_at AND selects share_text),
// that's not necessarily the one a given function cares about most. Checking
// for any of the three, instead of just the "obvious" one per call site,
// avoids a mismatch where the real error (e.g. "share_text does not exist")
// doesn't contain the substring a narrower check was looking for (e.g.
// "completed_at") and re-throws instead of degrading — seen live (2026-07-10):
// listCompletedReviews's select= list gets validated before its own
// completed_at filter, so a fresh pre-migration DB throws on "share_text",
// not "completed_at", and a check for only "completed_at" never fires.
function isMissingReviewColumn(err) {
  const msg = String(err.message);
  return msg.includes("share_text") || msg.includes("share_url") || msg.includes("completed_at");
}
...
/** Single pending_reviews row by id — the button-click approval flow (api/slack/interactivity.js) carries this in the button's value instead of re-deriving it from channel+approver. */
export async function getPendingReviewById(id) {
  const rows = await rest("pending_reviews", {
    query: `?id=eq.${encodeURIComponent(id)}&select=*`,
  });
  return rows[0] || null;
...
/**
 * Most recent NOT-YET-COMPLETED review for a specific subtask, regardless of
 * who was named as approver — used by the manager blanket-approval path
 * (2026-07-10): the project's Owner can approve a whole stage without being
 * individually @mentioned, but if someone DID already share a link for a
 * given subtask via the targeted flow, this pulls that context in for the
 * ClickUp comment instead of losing it.
 */
export async function getLatestPendingReviewForSubtask(subtask_id) {
  try {
    const rows = await rest("pending_reviews", {
      query: `?subtask_id=eq.${encodeURIComponent(subtask_id)}&completed_at=is.null&select=*&order=created_at.desc&limit=1`,
    });
    return rows[0] || null;
  } catch (err) {
    if (isMissingReviewColumn(err)) {
      const rows = await rest("pending_reviews", {
        query: `?subtask_id=eq.${encodeURIComponent(subtask_id)}&select=*&order=created_at.desc&limit=1`,
      });
      return rows[0] || null;
    }
    throw err;
