...
// Canonical team ordering — used to group deliverables consistently for
// lib/orchestrator.js's stage_plan (dashboard/Timeline tracking) and display
// purposes. NOT a gating order: every team's subtasks are assigned in
// parallel at creation time (2026-07-10) since teams often genuinely don't
// need to wait on each other (dev doesn't need design "approved" first to
// start building, etc.) — an earlier sequential content->design->dev->video
// handoff was tried and reverted for exactly this reason.
...
export async function createClickUpTickets(payload, owners) {
  ...
  for (let i = 0; i < cappedDeliverables.length; i++) {
    ...
    // Every team's subtasks get assigned immediately (2026-07-10: reverted
    // the earlier sequential content->design->dev->video_design gating --
    // teams often genuinely can start in parallel, e.g. dev doesn't need to
    // wait on design being "approved" first, so forcing that ordering was
    // actively wrong, not just a nice-to-have staging feature).
