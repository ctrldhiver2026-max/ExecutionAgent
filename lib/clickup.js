...
export async function getProjectLiveStatus(parentTaskId) {
    ...
    await Promise.all(
      stubs.map((s) =>
        clickupFetch(`/task/${s.id}`).catch((err) => {
          console.error(`[clickup] failed to fetch subtask ${s.id} for status (non-fatal)`, err);
          return null;
        })
      )
    )
  ...
  const assignees = Array.from(new Set(details.flatMap((t) => t.assignees)));
